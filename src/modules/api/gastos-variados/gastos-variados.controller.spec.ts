...
describe('GastosVariadosController', () => {
  ...
  describe('findAll', () => {
    it('should return an array of gastos variados', async () => {
      ...
      const gastos = [
        { id: 1, descricao: 'Aluguel', valor: '1200.00', data_pgto: new Date() },
        { id: 2, descricao: 'Internet', valor: '150.00', data_pgto: new Date() },
      ];
  ...
  describe('findOne', () => {
    it('should return a gasto variado by id', async () => {
      ...
      const gasto = { id: gasto_variado_id, descricao: 'Aluguel', valor: '1200.00', data_pgto: new Date() };
      ...
      expect(service.findOne).toHaveBeenCalledWith(+orcamento_id, +gasto_variado_id);
    });
  ...
  describe('update', () => {
    it('should update a gasto variado', async () => {
      ...
      const result = await controller.update(orcamento_id, gasto_variado_id, updateGastoDto);

      expect(result).toEqual(updatedGasto);
      expect(service.update).toHaveBeenCalledWith(+orcamento_id, +gasto_variado_id, updateGastoDto);
    });
  });
  ...
  describe('remove', () => {
    it('should perform a soft delete of a gasto variado', async () => {
      ...
      const gastoToDelete = { id: gasto_variado_id, descricao: 'Aluguel', valor: '1200.00', data_pgto: new Date() };
      ...
      expect(service.softDelete).toHaveBeenCalledWith(+orcamento_id, +gasto_variado_id);
    });
  });
});
