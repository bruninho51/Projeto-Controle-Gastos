...
export class LinhaDoTempoInvestimentosController {
    constructor(private readonly linhaDoTempoService: LinhaDoTempoInvestimentosService) {}
      ...
      create(
        ...
        @Body() createRegistroLinhaDoTempoDto: RegistroInvestimentoLinhaDoTempoCreateDto,
      ) {
        return this.linhaDoTempoService.create(+investimento_id, createRegistroLinhaDoTempoDto);
      }
    
      @Get()
      @ApiOperation({ summary: "Buscar todos os registros de linha do tempo do investimento" })
      @ApiResponse({ status: 200, description: "Lista de lançamentos na linha do tempo." })
      @ApiResponse({ status: 500, description: "Erro interno no servidor." })
      ...
      @ApiResponse({ status: 200, description: "Registro de linha do tempo encontrado." })
      @ApiResponse({ status: 404, description: "Registro de linha do tempo não encontrado." })
      @ApiResponse({ status: 500, description: "Erro interno no servidor." })
      findOne(
        @Param("investimento_id") investimento_id: string,
        @Param("id") id: string,
      ...
      update(
        @Param("investimento_id") investimento_id: string,
        @Param("id") id: string,
        @Body() updateRegistroLinhaDoTempoDto: RegistroInvestimentoLinhaDoTempoUpdateDto,
      ) {
      ...
      @ApiResponse({ status: 404, description: "Registro de linha do tempo não encontrado." })
      @ApiResponse({ status: 500, description: "Erro interno no servidor." })
      remove(@Param("investimento_id") investimento_id: string, @Param("id") id: string) {
